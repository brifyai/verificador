...
-- Create profiles table if it doesn't exist (safety)
...
CREATE POLICY "Admins can view their assignments" ON radio_assignments
    FOR SELECT USING (auth.uid() = user_id OR exists (select 1 from profiles where id = auth.uid() and role = 'super_admin'));
